...
namespace Migrator.Providers.SQLite
{
	...
	{
		public SQLiteTransformationProvider(Dialect dialect, string connectionString, int commandTimeout)
			: base(dialect, connectionString, commandTimeout)
		{
			_connection = new SqliteConnection(_connectionString) {ConnectionString = _connectionString};
			_connection.Open();
		}
